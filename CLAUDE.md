...
- **Ngày tháng ghi theo Nghị định 30/2020/NĐ-CP**: ngày < 10 và **tháng 1, 2** thêm số 0 phía trước, **từ tháng 3 trở đi ghi số trần** (`THÁNG 8 NĂM 2026`, không phải `THÁNG 08`). Dùng `viDay()` / `viMonth()` cho mọi chỗ in ra giấy: bìa tổng, bìa quyển, dòng *Công tác*, khối ký Tr1/Tr2, dòng ngày ở đầu trang ngày. **Riêng `bookFileBase` vẫn đệm 0 cho mọi tháng** (`T08-2026`) để tên file sắp đúng thứ tự.
...
- **Mục 4 và 5 in đủ 3 lựa chọn** Tốt / Bình thường / Kém, ô vuông vẽ bằng `border` + `transform` (`.opts .box`) chứ không dùng ký tự ☑ — tránh phụ thuộc font khi html2canvas rasterize. **Nét phải ≥1,5pt**: Chrome làm tròn `border-width` xuống px nguyên nên 0,7–1,2pt đều ra 1px = nét mảnh nhất, in ra nhìn mờ. 1,5pt cho đúng 2px — đo qua html2canvas: mực trong vùng ô tích tăng từ 16,6% lên 28,7%.
- **Mọi khối ký chỉ ghi họ tên trần** — `bareName()` cắt "Ông/Bà" đứng đầu, đúng như quyển mẫu. Áp cho: cuối **trang ngày** (`.daysign-name`), khối ký **bìa tổng** (`.sign-name`), cuối **Tr1/Tr2** (`.rep-name`). Ngược lại, chỗ **liệt kê** thì giữ nguyên "Ông …": bảng đại diện trên bìa quyển và bảng cán bộ Tr2. Chữ Ông nằm giữa tên ("Đặng Ông Minh") không bị cắt, và ô nhập trên màn hình cũng không đổi.
